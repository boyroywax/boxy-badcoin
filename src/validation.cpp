...
// Boxycoin - check algo and auxpow
...
CAmount GetBlockSubsidy(int nHeight, int nBits, const Consensus::Params& consensusParams)
{
    ...
    CAmount nMoneyLimit = 210000000 * COIN;
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            /* Boxycoin: we only use the lowest 8 bits for BIP9, so we mask the chainid and algo (0x00FFFF00)*/
